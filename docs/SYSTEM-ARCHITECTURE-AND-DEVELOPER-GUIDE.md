...
**Global chrome:** Public marketing pages use `components/site-shell.js` for a consistent sticky header and footer. The homepage (`app/page.js`) uses the premium homepage shell (`portal-shell--premium-home`) and composes sections in order: `HomeHeroPremium` (full-bleed), then in-content-rail sections (`HomeQuizEntrySection` with `id="quiz"`, buyer journey, destination cards, services, planning tools, deferred carousels, final CTA). Hash deep-links such as `/#quiz` and `/#popular-routes` are intentionally supported. Contact shortcuts in the header come from `components/site-header-meta-row.jsx` (email, phone, destination hub links, and social icons on wide viewports; mobile uses `SiteHeaderMobileUtilities`).
