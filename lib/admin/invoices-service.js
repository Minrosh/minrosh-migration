...
function readConfiguredBankDetails() {
  ...
  const accountName = String(process.env.INVOICE_BANK_ACCOUNT_NAME || file.accountName || "").trim();
  const bsb = String(process.env.INVOICE_BANK_BSB || file.bsb || "").trim();
  ...
  const accountNumberMasked = accountNumberRaw
    ...
    : String(file.accountNumberMasked || "").trim();
  const swift = String(process.env.INVOICE_BANK_SWIFT || file.swift || "").trim();
  const bankName = String(file.bankName || "").trim();
  const paymentReferencePrefix = String(file.paymentReferencePrefix || "INV").trim() || "INV";
  return { accountName, bsb, accountNumberMasked, swift, bankName, paymentReferencePrefix };
}
...
export function createInvoice({
  ...
  return withInvoiceMutationLock(() => {
    ...
    const fromTemplate = templateId
      ? (readInvoiceTemplates().templates || []).find((t) => t.id === templateId)
      : null;
    let lines = sanitizeLines(lineItems, service);
    ...
    const row = {
      ...
      terms: String(terms || fromTemplate?.defaultTerms || "Payment due within 7 days.").slice(0, 1000),
      notes: String(notes || "").slice(0, 2000),
      recurringRuleId: String(recurringRuleId || "").trim() || undefined,
      paymentInstructions: readConfiguredBankDetails(),
      issuer: fromTemplate?.issuer || undefined,
...
export function updateInvoice(id, patch = {}) {
  return withInvoiceMutationLock(() => {
    ...
    if (patch.lineItems) {
      ...
      Object.assign(next, { lineItems: taxed, ...calculateTotals(taxed), amount: calculateTotals(taxed).total });
    }
    invoices[i] = next;
    writeInvoices({ invoices });
    return next;
...
export function upsertTemplate(template) {
  return withInvoiceMutationLock(() => {
    ...
    const row = {
      ...
      issuer: template.issuer && typeof template.issuer === "object" ? template.issuer : {},
      lineDefaults: Array.isArray(template.lineDefaults) ? template.lineDefaults : [],
      updatedAt: new Date().toISOString(),
    };
...
export function setBankDetails(input) {
  const next = {
    ...
    bsb: String(input.bsb || "").trim(),
    accountNumberMasked: maskAccountNumber(String(input.accountNumberMasked || input.accountNumber || "")),
    swift: String(input.swift || "").trim(),
    bankName: String(input.bankName || "").trim(),
    paymentReferencePrefix: String(input.paymentReferencePrefix || "INV").trim() || "INV",
  };
