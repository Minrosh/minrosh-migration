...
/**
 * Remove all uploaded files for a customer (private storage only).
 * @param {{ uploadFolder?: string, id: string }} customer
 */
export function deleteCustomerUploadFolder(customer) {
  const dir = getPrivateCustomerDir(customer);
  if (!fs.existsSync(dir) || !fs.statSync(dir).isDirectory()) return;
  if (!isPathInsideRoot(storageUploadsDir, dir)) return;
