 ...
 * If the password lives only in admin-auth.json, you must still set ADMIN_SESSION_SECRET
 * (or a non-placeholder ADMIN_PASSWORD) so sessions can be issued and verified.
 */
export function hasAdminSessionSigningSecret() {
  ...
  return !isPlaceholder(envPassword);
}
