...
export function appendSocialSentimentEvent(fields) {
  const platform = String(fields.platform || "social");
  const commentText = summarizeBody(fields.commentText, 600);
  return appendIntelligenceEvent({
    type: "social_sentiment_signal",
    actor: "social_webhook",
    country: "",
    sourceName: `${platform} sentiment`,
    sourceUrl: "",
    contentHash: "",
    postId: String(fields.postId || ""),
    commentId: String(fields.commentId || ""),
    platform,
    commentText,
    sentimentScore: fields.sentimentScore,
    negative: fields.negative,
    labels: Array.isArray(fields.labels) ? fields.labels : [],
    leadIntentScore: fields.leadIntentScore,
    hotLead: fields.hotLead,
    crmLeadId: String(fields.crmLeadId || ""),
  });
}
      ...
      grounding:
        payload.grounding && typeof payload.grounding === "object"
          ? {
              ...
              excerpt: summarizeBody(payload.grounding.excerpt || payload.summary || "", 900),
              extractedAt: payload.grounding.extractedAt || now,
              ...(Array.isArray(payload.grounding.articleUrls)
                ? { articleUrls: payload.grounding.articleUrls.slice(0, 50) }
                : {}),
              ...(typeof payload.grounding.aggregatedChars === "number"
                ? { aggregatedChars: payload.grounding.aggregatedChars }
                : {}),
              ...(payload.grounding.gemini === true ? { gemini: true } : {}),
              ...(payload.grounding.geminiError
                ? { geminiError: String(payload.grounding.geminiError).slice(0, 500) }
                : {}),
            }
          : null,
      moderationNote: "",
      publishedAt: null,
      provenance: Array.isArray(payload.provenance) ? payload.provenance.slice(0, 200) : [],
    };
