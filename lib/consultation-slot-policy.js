...
function readMinAdvanceHours() {
  const raw = String(process.env.CONSULTATION_MIN_LEAD_HOURS || "").trim();
  if (!raw) return DEFAULT_MIN_ADVANCE_HOURS;
  const parsed = Number(raw);
  if (!Number.isFinite(parsed)) return DEFAULT_MIN_ADVANCE_HOURS;
  return Math.max(0, Math.min(168, Math.floor(parsed)));
}

export const CONSULTATION_MIN_ADVANCE_HOURS = readMinAdvanceHours();
...
export function validateConsultationSlot(input) {
  ...
  const minStartMs = Date.now() + CONSULTATION_MIN_ADVANCE_HOURS * 60 * 60 * 1000;
  if (instant.getTime() < minStartMs) {
    return {
      ok: false,
      error: `Please choose a date and time at least ${CONSULTATION_MIN_ADVANCE_HOURS} hours from now.`,
    };
  }
