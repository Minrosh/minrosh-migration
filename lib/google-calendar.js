...
function buildStartDate(contact) {
...
function buildSlot(contact) {
  const start = buildStartDate(contact);
  if (!start || Number.isNaN(start.getTime())) return null;
  const duration = Number(contact.consultationDurationMins || 45);
  const durationMins = Number.isFinite(duration) ? Math.max(30, Math.min(60, duration)) : 45;
  const end = new Date(start.getTime() + durationMins * 60 * 1000);
  const timeZone = String(contact.timeZone || "Australia/Brisbane").trim() || "Australia/Brisbane";
  return { start, end, durationMins, timeZone };
}
