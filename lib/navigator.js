...
export function buildNavigatorRecommendation(answers) {
  ...
  const strategyPlan = 
    /study/i.test(goal) ? "Study followed by regional work experience." :
    /skilled/i.test(goal) ? "Direct entry combined with employer sponsorship options." :
    "Applying for a partner visa while in Australia.";
...
export function navigatorSummaryText(answers, recommendation) {
  return [
    ...
    `Primary Strategy: ${recommendation.strategyPlan}.`,
    `Secondary Option: ${recommendation.backupStrategy.need}.`,
  ].join(" ");
}
