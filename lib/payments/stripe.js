...
/**
 * Consultations are billed as fixed {@link CONSULTATION_SLOT_DURATION_MINS}-minute sessions only.
 * `durationMins` from the client is ignored for pricing (tamper-resistant alignment with slot policy).
 */
export function consultationChargeAmountCents(_durationMins, offer = "first_15_free") {
  const basePerMinute = 8; // AUD cents unit -> $8/min baseline strategic consult
  const duration = CONSULTATION_SLOT_DURATION_MINS;
