...
function validateNavStore(raw) {
  ...
  return {
    ...
    data: {
      ...
      headerCtaLabel: String(raw.headerCtaLabel || "").trim(),
      headerCtaHref: String(raw.headerCtaHref || "").trim(),
...
export function writeNavigationSettings(patch, updatedBy) {
  return withWebsiteStoreMutation(websiteNavigationFile, () => {
    const current = getNavigationSettings();
    const next = validateNavStore({
      schemaVersion: 1,
      primaryLinks: patch.primaryLinks ?? current.primaryLinks,
      headerCtaLabel: patch.headerCtaLabel ?? current.headerCtaLabel,
      headerCtaHref: patch.headerCtaHref ?? current.headerCtaHref,
