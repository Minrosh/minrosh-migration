...
export function getUploadSessionSecret() {
  const dedicated = String(process.env.UPLOAD_SESSION_SECRET || "").trim();
  if (dedicated) return dedicated;
  const smsOn = String(process.env.UPLOAD_LINK_SMS_VERIFICATION || "").toLowerCase() === "true";
  if (smsOn && process.env.NODE_ENV === "production") {
    return "";
  }
