...
const SLUG = "/post-study-visa-australia";

describe("post-study-visa-australia CMS route fallback", () => {
  afterEach(() => {
  ...
  it("returns published sections when CMS flag is on and page is published", () => {
    ...
    saveWebsitePageDraft(SLUG, {
      pageTitle: "Post-Study Visa",
      sections: [
        {
          id: "hero-post-study",
          type: "hero",
          props: {
            heading: "CMS Post-Study visa heading",
    ...
    expect(rendered?.sections?.[0]?.props?.heading).toBe("CMS Post-Study visa heading");
  });
