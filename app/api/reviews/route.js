export async function GET() {
  ...
  if (!apiKey || !placeId) {
    return Response.json({ reviews: [], reason: "places_not_configured" });
  }
  try {
    ...
    if (!res.ok) {
      return Response.json({ reviews: [], reason: "places_http_error" }, { status: 502 });
    }
    const data = await res.json();
    if (data?.status && data.status !== "OK") {
      const status = String(data.status || "").toLowerCase();
      return Response.json({ reviews: [], reason: `places_${status || "status_error"}` }, { status: 502 });
    }
    ...
    return Response.json({ reviews: [], reason: "places_request_failed" }, { status: 502 });
  }
}
